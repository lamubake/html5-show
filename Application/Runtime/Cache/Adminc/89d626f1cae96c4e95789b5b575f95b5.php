...
<div class="container-fluid">
  ...
  <div class="row-fluid">
					...
					<div class="span12 blog-page">
						...
						<h3 class="page-title">
							...
							广告管理
							 ...
							 <small>Advertising management</small>
               ...
               <a class="btn green" href='?c=ad&a=add' style='margin-bottom: 20px;'> 填加广告</a>
 ...
 //搜索用户
	...
	<div class="portlet-body" style=" margin-top:15px;">
														<table class="table table-striped table-bordered table-advance table-hover">
															<thead>
																<tr>
                            <th scope="col" width="3%"><i class="icon-briefcase"></i>ID</th>
                              <th align="center" scope="col" abbr="lbtnOperate"><i class="icon-bookmark"></i>操作</th>
                               <th scope="col" width="20%">名称</th>
                              <th scope="col" width="15%"><i class="icon-question-sign"></i>类型</th>
                            
                            <th scope="col" width="15%"><i class="icon-bookmark"></i>缩略图</th>
                            
                            <th scope="col" width="15%"><i class="icon-bookmark"></i>添加时间</th>
                        </tr>
                              ...
                              <?php if(is_array($select)): foreach($select as $key=>$item): ?><tr>
                                    ...
                                    <td align="center" style="width:20px;"> <a href="?c=ad&a=edit&id=<?php echo ($item["id"]); ?>"class="btn blue"><i class=" icon-edit"> </i>编辑 </a>  <a href="javascript:;" onClick="javascript:if(confirm('你确信要删除[<?php echo ($item["name"]); ?>]吗？')) window.location='?c=ad&a=del&id=<?php echo ($item["id"]); if(($flag) == "useranli"): ?>&flag=useranli<?php endif; ?>'"class="btn"><i class="icon-remove icon-white"></i> 删除</a>
                                      </if>
                                    </div></td>
                                   <td><?php echo ($item["name"]); ?> </td>
                                    <td> 
                                  	<?php switch($item["type"]): case "2": ?>首页大图<?php break;?> 
                                        <?php case "1": ?>首页logo<?php break;?>  
                                    <?php case "3": ?>场景浏览背景<?php break;?>                                
                                    
                                  		<?php default: ?>其它图<?php endswitch;?>
                                  	
                                  	 </td>
                             
                                    <td>
                                    <?php if(($item["filetype_int"]) == "2"): ?><audio src="/Uploads/<?php echo ($item["url"]); ?>" controls="controls"></audio> <?php else: ?>  <img src="/Uploads/<?php echo ($item["url"]); ?>" class="thbnail"><?php endif; ?>
