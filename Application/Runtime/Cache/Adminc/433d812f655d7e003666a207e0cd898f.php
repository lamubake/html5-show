...
<div class="container-fluid">
  ...
  <div class="row-fluid">
               ...
               <div class="tabbable tabbable-custom tabbable-full-width">
			      ...
			      <li class="<?php echo ($ui["Ad_msgList"]); ?>"> <a href="<?php echo U('Ad/msgList');?>"> 公告管理</a> </li>
		         <li class="<?php echo ($ui["Ad_index"]); ?>"> <a href="<?php echo U('Ad/index');?>"> 广告管理</a> </li>
							</ul>
                        
                </div>
				
               <a class="btn green" href='?c=ad&a=msgadd' style='margin-bottom: 20px;'> 填加公告</a>
...
function checkAll(o){
 ...
 //搜索用户
     $(document).ready(function(){
		...
		var fn = $("#filetype_int:input").val();
        var top = $("#filetype_int2:input").val();
        $.get("adminc.php?c=file", {fnid:fn, top:top},function(data){});
 });   
 });
	...
	<div class="portlet-body" style=" margin-top:15px;">
														<table class="table table-striped table-bordered table-advance table-hover">
															<thead>
																<tr>
                            <th scope="col" width="67"><i class="icon-briefcase"></i>ID</th>
                              <th width="91" align="center" scope="col" abbr="lbtnOperate"><div align="center"><i class="icon-bookmark"></i>操作</div></th>
                              <th scope="col" width="54"><i class="icon-question-sign"></i>类型</th>
                            <th scope="col" width="179"><div align="center"><i class="icon-bookmark"></i>标题</div></th>
                            <th scope="col" width="78"><div align="left"><i class="icon-bookmark"></i>状态</div></th>
                            <th scope="col" width="258"><div align="center"><i class="icon-bookmark"></i>添加时间</div></th>
                        </tr>

															</thead>

															<tbody>
                              <?php if(is_array($msglist)): foreach($msglist as $key=>$item): ?><tr>
                                    <td align="center"> <?php echo ($item["id"]); ?> </td>
                                    <td align="center" style="width:20px;"> <div align="center"><a href="?c=ad&a=msgedit&id=<?php echo ($item["id"]); ?>" class="btn blue">  <i class=" icon-edit"> </i> 编辑 </a>  <a href="javascript:;" onClick="javascript:if(confirm('你确信要删除[<?php echo ($item["title"]); ?>]吗？')) window.location='?c=ad&a=msgdel&id=<?php echo ($item["id"]); if(($flag) == "useranli"): ?>&flag=useranli<?php endif; ?>
                                    '"class="btn"><i class="icon-remove icon-white"></i> 删除</a>
                                      </if>
                                    </div></td>
                                  <td> 
                                  	<?php switch($item["bizType"]): case "1": ?><div align="center">公告</div><?php break;?> 
                                        <?php case "2": ?><div align="center">通知</div><?php break;?>  
                               		  <div align="center"><?php default: ?>
                               		    其它</div><?php endswitch;?>
                                  	
                               	  </td>
                             
                                    <td>
                                      <div align="center"><?php echo ($item["title"]); ?>
                                   
                                    </div></td>
                                   <td><?php if(($item["status"]) == "1"): ?><div align="center" class="btn icn-only green">启用
                                       <?php else: ?> 
                                       <i class="btn black"><i class="icon-remove icon-white"></i>禁用</div><?php endif; ?></td> 
                                  <td><div align="center"><?php echo (date('Y-m-d H:i:s',$item["sendTime"])); ?> </div></td>
                                </tr><?php endforeach; endif; ?>
                                <tr>
                                <td colspan="11">  </td>  </tr>
                    </table><div class="pages"><?php echo ($page); ?></div>               
</div>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function(){
    $("#fnid").bind("change",function(){
	 var url = $("#fnid").val();
     $.get("/adminc.php?c=ad",{fnid:url},function(){   
