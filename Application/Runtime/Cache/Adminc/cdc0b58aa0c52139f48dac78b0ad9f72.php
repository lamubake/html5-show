		        ...
		        <li class="<?php echo ($ui["sys_add"]); ?>"> <a href="<?php echo U('sys/add');?>"> 添加管理员</a> </li>
                 <li class="<?php echo ($ui["sys_edit"]); ?>"> <a href="/adminc.php?c=sys&a=edit&id=<?php echo ($Adminuserid); ?>"> 修改密码</a> </li>
										...
										<div class="media">
											...
											<img src="Public/media/image/5.jpg" alt=""  class="media-object" style="width:60px; height:60px">

											</a>

											<div class="media-body">

												<h4 class="media-heading">用户ID：<strong class="text-error">{<?php echo ($user["userid"]); ?>}</strong>   登陆名：<strong class="text-error"><?php echo ($user["username"]); ?></strong><span  class="pull-right"><a href="/adminc.php?c=sys&a=edit&id=<?php echo ($user["userid"]); ?>" class="btn blue "><i class="icon-remove icon-pencil"></i>  修改</a><a href="/adminc.php?c=sys&a=del&id=<?php echo ($user["userid"]); ?>" class="btn black"><i class="icon-remove icon-white"></i>  删除</a></span>	
<span><a  href="/adminc.php?c=user&a=e&id=<?php echo ($user["userid_int"]); ?>" class="btn purple-stripe">状态：</a>
<?php if(($user['useris']) == "1"): ?><a href="/adminc.php?c=user&a=e&id=<?php echo ($user["userid_int"]); ?>" class="btn icn-only green"><i class="icon-play"></i></a><?php endif; ?>
<?php if(($user['useris']) == "0"): ?><a href="/adminc.php?c=user&a=e&id=<?php echo ($user["userid_int"]); ?>" class="btn icn-only red"><i class="icon-remove"></i></a><?php endif; ?>
					</span></h4>

												<p>登录次数：<?php echo ($user["loginnum"]); ?> ，最后一次登录时间：<?php if($user[logintime] >0): ?>  <?php echo (date('Y-m-d H:i',$user["logintime"])); else: ?> 未曾登录过<?php endif; ?>角色：<?php if(($user['role']) == "1"): ?>系统管理员<?php else: ?> 普通管理员<?php endif; ?> </p>
