...
<div class="container-fluid">
	...
	<!-- END PAGE HEADER-->
			...
			<form class="login-form" action="<?php echo U('user/index');?>" method="post">
			
				
				根据用户名搜索&nbsp(支持模糊查询)&nbsp:&nbsp&nbsp&nbsp <input type="text" name="username" id="id"  value="" />
			
			&nbsp&nbsp&nbsp <input type="submit" name="button" id="button" value="搜索">
			</form>
				...
				<!-- END PAGE HEADER-->
										...
										<div class="portlet-body form ">
                                         ...
                                         <?php if(is_array($select)): foreach($select as $key=>$user): ?><div class="media-body">
										...
										<div class="media">
<a href="/adminc.php?c=user&a=e&id=<?php echo ($user["userid_int"]); ?>"class="pull-left"><img src="<?php if(substr($user['headimg'],0,7) != 'http://'): ?>/Uploads/<?php endif; echo ((isset($user["headimg"]) && ($user["headimg"] !== ""))?($user["headimg"]):"defaultuser.jpg"); ?>" width="60" height="60" class="media-object" style=" width:60px; height:60px"/> </a>


											<div class="media-body">

												<h4 class="media-heading">
                                                  <strong class="btn purple-stripe">登陆名：<?php echo ($user["email_varchar"]); ?></strong>   
                                                 <strong class="btn purple-stripe">角色：<?php echo ($user["gname"]); ?></strong>
                                                 <strong class="btn purple-stripe">秀点数：<?php echo ($user["xd"]); ?></strong>
                                                 <strong class="btn purple-stripe">可创建场景数：<?php echo ($user["allow_nums"]); ?></strong>
					<span><a  href="/adminc.php?c=user&a=e&id=<?php echo ($user["userid_int"]); ?>" class="btn purple-stripe">状态：</a><?php if(($user['status_int']) == "1"): ?><a href="/adminc.php?c=user&a=e&id=<?php echo ($user["userid_int"]); ?>" class="btn icn-only green"><i class="icon-play"></i></a><?php endif; if(($user['status_int']) == "0"): ?><a href="/adminc.php?c=user&a=e&id=<?php echo ($user["userid_int"]); ?>" class="btn icn-only red"><i class="icon-remove"></i></a><?php endif; ?>
					</span>
                                                ...
                                                <span  class="pull-right"><a  href="/adminc.php?c=user&a=e&id=<?php echo ($user["userid_int"]); ?>"  class="btn blue"><i class="icon-remove icon-pencil"></i>  修改</a><a href="/adminc.php?c=user&a=del&id=<?php echo ($user["userid_int"]); ?>" class="btn black"><i class="icon-remove icon-white"></i>  删除</a></span></h4>

												<p>
                                                 <span class="btn mini green-stripe">用户ID：<?php echo ($user["userid_int"]); ?></span>
                                                <span class="btn  mini  green-stripe">用户名：<?php echo ($user["uname"]); ?></span><span class="btn mini green-stripe">注册时间：<?php echo ($user["create_time"]); ?> </span><span class="btn mini green-stripe">到期时间:<?php if($user['end_time']>0): echo (date('Y-m-d H:i',$user["end_time"])); else: ?> 永久<?php endif; ?> </span></p>
